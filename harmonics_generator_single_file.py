...
def generate_and_save_harmonics_single_file(fundamental_freq, num_harmonics=63, sample_rate=44100, duration=3):
    all_harmonics = np.array([], dtype=np.float32)

    for n in range(1, num_harmonics + 1):
        harmonic_freq = fundamental_freq * n
        waveform = generate_sine_wave(harmonic_freq, sample_rate, duration)
        all_harmonics = np.concatenate((all_harmonics, waveform))

    save_wave_to_file(all_harmonics, f"{fundamental_freq}_harmonics.wav", sample_rate)

# Example usage with a fundamental frequency of 78 Hz
generate_and_save_harmonics_single_file(78)
